...
FAADE.CONFIG = {
    ...
    'gdocs_key': '0ArYmNHuRadHbdEFzQmZzUHl1dUV0VGgzZ1h6NE5NY0E',
    'gform_key': 'dEFzQmZzUHl1dUV0VGgzZ1h6NE5NY0E6MQ',
    'gdocs_development_key': '0ArYmNHuRadHbdFlCY25wNWY4ZGFzSmNFa3prQUVvRGc',
    'gform_development_key': '0ArYmNHuRadHbdFlCY25wNWY4ZGFzSmNFa3prQUVvRGc',
};

// Utility debugging function.
FAADE.log = function (msg) {
    if (!FAADE.CONFIG.debug) { return; }
    if (console && console.log) { console.log('FETLIFE FAADE: ' + msg); }
...
FAADE.broadcastNewProximalReports = function (doc) {
    ...
    if (last_timestamp_checked < latest_timestamp_filed) {
        ...
        for (var i = rows.length - num_reports; i <= rows.length - 1; i++) {
            ...
            report_loc = rows[i].childNodes[5].textContent;
        ...
        if (reports_to_alert.length) {
            ...
            document.querySelector('.btnsqr[data-closes-modal="faade"]').addEventListener('click', function () {
                for (var i = 0; i < reports_to_alert.length; i++) {
                    ...
                    GM_openInTab(url + reports_to_alert[i].childNodes[1].textContent.match(/\d+/)[0]);
                }
            });
        }
    }
...
FAADE.main = function () {
    ...
    if (window.location.href.match(/details\.html\?u=(\w+)$/)) {
        ...
        if (-1 !== abuser_ids.indexOf(id_in_url)) {
            ...
            for (var iy = 0; iy < abuse_reports.length; iy++) {
                ...
                var details_html = '<ul><li class="faade_abuse_report_datetime">' + abuse_reports[iy].childNodes[6].innerHTML + '</li>';
                details_html += '<li class="faade_abuse_report_location">' + abuse_reports[iy].childNodes[5].innerHTML + '</li></ul>';
                ...
                tr.innerHTML += '<td width="50%">' + abuse_reports[iy].childNodes[4].innerHTML + '</td>';
                report_el.appendChild(tr);
            }

            var before = document.querySelector('#dealbreaker_info');
    ...
    for (i = 0; i < user_links.length; i++) {
        ...
        var id = user_links[i].textContent.trim();

        // Get nickname.
        var n;
        if (user_links[i].children.length) {
            // This is a '.username a' link, not a text link.
            n = user_links[i].textContent.trim();
        } else {
